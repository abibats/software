...
2. 直接基于"参考资料"回答，不要编造不存在的教室、座位或预约。
3. 直接回答用户问题，不要写系统概览、功能介绍或无关流程。
4. 回答尽量控制在 150 字以内；如列座位，最多列 5 个。
5. 不要输出 JSON，不要暴露内部字段名。
""".strip()
...
def build_available_seats(db, user, text):
    ...
    return available, seat_filters, time_window


def build_context_text(db, user, text):
    lines = [f"当前日期：{today_text()}"]
    lines.append(f"当前用户：{user['display_name']}（{user['department']}，{user['role_name']}）")
    ...
    lines.append(f"系统共有 {active_rooms} 间活跃自习室，{active_seats} 个活跃座位。")
    ...
    if is_seat_query:
        available, filters, time_window = build_available_seats(db, user, text)
        total = len(available)
        filter_desc = ""
        if filters:
            labels = []
            ...
            if filters.get("quiet_zone"):
                labels.append("安静区")
            filter_desc = "（筛选条件：" + "、".join(labels) + "）"
        if time_window:
            lines.append(f"查询时段：{time_window[0][5:16]} 到 {time_window[1][11:16]}")
        lines.append(f"查询结果{filter_desc}：共找到 {total} 个可用座位。")
        if available:
            lines.append("部分可用座位列表：")
            for s in available[:5]:
                ...
                if s["quiet_zone"]:
                    tags.append("安静区")
                tag_str = "、".join(tags) if tags else "普通座"
                lines.append(f"  - {s['room_name']} 座位{s['code']}（{s['building']}，{tag_str}，开放{s['open_time']}-{s['close_time']}）")
            if total > 5:
                lines.append(f"  ...还有 {total - 5} 个座位未列出。")
        else:
            lines.append("当前没有找到符合条件的可用座位，建议放宽筛选条件或更换时段。")
    ...
    if my_reservations:
        lines.append(f"当前用户的有效预约（{len(my_reservations)}条）：")
        for r in my_reservations:
            lines.append(f"  - {r['room_name']} {r['seat_code']}，{r['start_time']} 到 {r['end_time']}，状态：{r['status']}")
    else:
        lines.append("当前用户暂无有效预约。")

    lines.append("系统规则：预约开始时间必须是未来整点；单次预约最长4小时；到达后输入教室动态编码签到；开始后15分钟未签到自动取消并记录违约。")
    return "\n".join(lines)
...
def call_openai_assistant_api(db, user, text, api_url, api_key, model):
    payload = {
        ...
        "messages": [
            ...
            {
                "role": "user",
                "content": "用户问题：" + text + "\n\n参考资料：\n" + build_context_text(db, user, text),
            },
        ],
        "temperature": 0.2,
